...
let closedSetStart
let closedSetEnd
// sourceColor = color(0, 255, 0)
// destColor = color(255, 0, 0)
...
function resetCanvas() {
    ...
    destination.show(color(255, 0, 0));
    noLoop();
    console.log(openSet)
}
...
function Node(i, j) {
    ...
    this.clicked = () => {
        if (sourceSelected) {
            // if(this == source){
            this.show(color(0, 255, 0))

            // source = this
            // srcORdstClicked = false
        }
        else if (destinationSelected) {
            this.show(color(255, 0, 0))
        }
        else if (!this.obstacle) {
            this.obstacle = true;
            this.show(color(11,110,169,255));
        }
        // else{
        //     this.obstacle = false;
...
function bidirectionalInitialize() {
    // Initialize open and closed sets for both searches
    openSetStart = [];
    openSetEnd = [];
    closedSetStart = [];
    closedSetEnd = [];

    openSetStart.push(source);
    openSetEnd.push(destination);
    closedSetStart.push(source);
    closedSetEnd.push(destination);
}
...
function start() {
    if (algo === null) {
        let startButton = document.getElementById('startButton')
        startButton.innerHTML = `Select An Algorithm!`
        return
    }
